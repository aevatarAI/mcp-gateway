...
public class AdapterReverseProxyController(
    ...
    ISessionRoutingHandler sessionRoutingHandler) : ControllerBase
{
    ...
    private readonly IAdapterSessionStore sessionStore =
        sessionStore ?? throw new ArgumentNullException(nameof(sessionStore));

    private readonly ISessionRoutingHandler sessionRoutingHandler =
        sessionRoutingHandler ?? throw new ArgumentNullException(nameof(sessionRoutingHandler));
    ...
    public async Task ForwardStreamableHttpRequest(string name, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrEmpty(sessionId))
            targetAddress = await sessionRoutingHandler
                .GetNewSessionTargetAsync(name, HttpContext, cancellationToken).ConfigureAwait(false);
        else
            targetAddress = await sessionRoutingHandler
                .GetExistingSessionTargetAsync(HttpContext, cancellationToken).ConfigureAwait(false);
        ...
        var proxiedRequest = await HttpProxy.CreateProxiedHttpRequestAsync(HttpContext,
            uri => ReplaceUriAddress(uri, targetAddress), cancellationToken);
        ...
        await HttpProxy.CopyProxiedHttpResponseAsync(HttpContext, response, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    {
        var targetAddress = await sessionRoutingHandler
            .GetExistingSessionTargetAsync(HttpContext, cancellationToken).ConfigureAwait(false);
    ...
    public async Task ForwardSseRequest(string name, CancellationToken cancellationToken)
    {
        ...
        await StreamProxiedSseHttpResponseAsync(HttpContext, name, response, targetAddress, cancellationToken)
            .ConfigureAwait(false);
    }

    private async Task StreamProxiedSseHttpResponseAsync(HttpContext context, string name,
        HttpResponseMessage response, string targetAddress, CancellationToken cancellationToken)
    {
        ...
        {
            var chunkLength = await stream.ReadAsync(buffer.AsMemory(0, buffer.Length), cancellationToken)
                .ConfigureAwait(false);
            if (chunkLength > 0)
            {
                ...
                if (sessionId == null)
                {
                    ...
                    if (index >= 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(sessionId))
                        {
                            await sessionStore.SetAsync(sessionId, targetAddress, cancellationToken)
                                .ConfigureAwait(false);
                        }

                        await context.Response.Body.WriteAsync(Encoding.UTF8.GetBytes(modified), cancellationToken)
                            .ConfigureAwait(false);
                    }
                }
                else
